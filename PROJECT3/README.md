...
├── visualize.py                     ← LOCAL: Matplotlib + Seaborn + Plotly plots
├── requirements.txt
...
Download the UQ Vital Signs dataset and unzip it so you have:
```
downloaded_data/
...
Run the preparation script to flatten + sample to ~1.5 GB:
```bash
...
(same approach as Project 1, but sampled to 1.5 GB instead of the full 4.14 GB).
...
3. Split **70% train / 15% val / 15% test** (random, seed=42)
