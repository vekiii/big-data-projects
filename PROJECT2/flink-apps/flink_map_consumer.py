#!/usr/bin/env python3
"""
Flink Kafka Consumer + Folium Map Visualizer
===========================================
Consumes aggregated pollution results from 'flink-results' Kafka topic
and renders an interactive HTML map using Folium.

This is the Flink counterpart of `spark-apps/map_consumer.py` and expects
records produced by the Flink processor job (fields like CO2_avg, CO2_max,
window_start/window_end, lat/lon etc.).

Usage:
    python flink_map_consumer.py \
        --broker localhost:9092 \
        --pollutant CO2 \
        --output flink_pollution_map.html \
        --refresh 15
...
def build_popup_html(record: dict, pollutant: str) -> str:
    ...
    for p in all_p:
        ...
        if avg_key in record:
            ...
            lines.append(f"<b>{p}</b>: avg={record[avg_key]:.2f} | max={record[max_key]:.2f} {unit}<br>")
    return "".join(lines)


def render_map(grid_data: dict, pollutant: str, output_path: str, refresh_seconds: int, window_info: dict):
    m = folium.Map(location=MADRID_CENTER, zoom_start=DEFAULT_ZOOM, tiles="CartoDB positron")
    ...
    if refresh_seconds > 0:
        m.get_root().html.add_child(folium.Element(f'<meta http-equiv="refresh" content="{refresh_seconds}">'))
    ...
    print(f"[Flink Map] Rendered {plotted} grid cells | pollutant={pollutant}")
    m.save(output_path)
    print(f"[Flink Map] Saved → {output_path}")


def consume_and_visualize(broker: str, pollutant: str, output_path: str, refresh_seconds: int):
    ...
    print(f"[Flink Map Consumer] Connected to Kafka broker: {broker}")
    print(f"[Flink Map Consumer] Listening on topic: flink-results")
    print(f"[Flink Map Consumer] Visualizing pollutant: {pollutant}")
    ...
    try:
        ...
        consumer.close()
        print(f"[Flink Map Consumer] Final map saved at: {output_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Flink Pollution Map Consumer")
    parser.add_argument("--broker", default="localhost:9092")
    parser.add_argument("--pollutant", default="CO2",
                        choices=["CO2", "CO", "HC", "NOx", "PMx", "noise", "all"])
    parser.add_argument("--output", default="flink_pollution_map.html")
